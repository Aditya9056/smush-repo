  ...
  */
	...
	$('input#wp-smush-save-settings').on('click', function (e) {
		...
		if ('' != setting_type && 'network' == setting_type) {
			...
			if ((strip_exif === null || !strip_exif.checked) && (super_smush === null || !super_smush.checked) && (smush_original === null || !smush_original.checked) && (resize_images === null || !resize_images.checked) && (smush_pngjpg === null || !smush_pngjpg.checked)) {
...
var Smush = function () {
  ...
  */
		...
		value: function call_ajax() {
			...
			this.request = Smush.ajax(this.is_bulk_resmush, this.current_id, this.url, nonce_value).done(function (res) {
				...
				if ('undefined' === typeof res.success || 'undefined' !== typeof res.success && false === res.success && 'undefined' !== typeof res.data && 'limit_exceeded' !== res.data.error) {
					...
					var error_msg = Smush.prepare_error_row(res.data.error_message, res.data.file_name, res.data.thumbnail, self.current_id, self.smush_type);
   ...
   * @returns {string}
   */
		value: function prepare_error_row(errorMsg, fileName, thumbnail, id, type) {
			...
			var tableDiv = '<div class="smush-bulk-error-row">' + '<div class="smush-bulk-image-data">' + thumbDiv + '<span class="smush-image-name">' + fileLink + '</span>' + '<span class="smush-image-error">' + errorMsg + '</span>' + '</div>';

			if ('media' === type) {
				tableDiv = tableDiv + '<div class="smush-bulk-image-actions">' + '<button type="button" class="sui-button-icon sui-tooltip sui-tooltip-constrained sui-tooltip-top-left smush-ignore-image" data-tooltip="' + wp_smush_msgs.error_ignore + '" data-id="' + id + '">' + '<i class="sui-icon-eye-hide" aria-hidden="true"></i>' + '</button>' + '</div>';
			}
