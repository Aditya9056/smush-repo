...
__webpack_require__(/*! ./modules/helpers */ "./_src/js/modules/helpers.js");
...
__webpack_require__(/*! ./modules/directory-smush */ "./_src/js/modules/directory-smush.js");
__webpack_require__(/*! ./modules/cdn */ "./_src/js/modules/cdn.js");
 ...
 */
__webpack_require__(/*! ./modules/notice */ "./_src/js/modules/notice.js");
...
var _smush = __webpack_require__(/*! ../smush/smush */ "./_src/js/smush/smush.js");

var _smush2 = _interopRequireDefault(_smush);
...
jQuery(function ($) {
  ...
  */
	var process_smush_action = function process_smush_action(e, current_button, smush_action, action) {
		...
		_smush2.default.progress_bar(current_button, wp_smush_msgs[action], 'show');

		//Restore the image
		$.post(ajaxurl, params, function (r) {

			_smush2.default.progress_bar(current_button, wp_smush_msgs[action], 'hide');
			...
			if (r.success && 'undefined' != typeof r.data.button) {
				...
				if ('undefined' != typeof r.data && 'restore' === action) {
					_smush2.default.update_image_stats(r.data.new_size);
				}
			} else {
				if (r.data.message) {
					//show error
	...
	var run_re_check = function run_re_check(button, process_settings) {
		...
		$.get(ajaxurl, params, function (r) {
			...
			if ('undefined' != typeof r.data) {
				...
				_smush2.default.update_stats(scan_type);
			}
		}).always(function () {
  ...
  */
	...
	$('body').on('click', '.wp-smush-send:not(.wp-smush-resmush)', function (e) {
		// prevent the default action
		e.preventDefault();
		new _smush2.default($(this), false);
	});
	...
	$('body').on('click', '.wp-smush-nextgen-send', function (e) {
		// prevent the default action
		e.preventDefault();
		new _smush2.default($(this), false, 'nextgen');
	});
	...
	$('body').on('click', '.wp-smush-nextgen-bulk', function (e) {
		...
		if ('undefined' === typeof wp_smushit_data || wp_smushit_data.unsmushed.length === 0 && wp_smushit_data.resmush.length === 0) {
			return false;
		}

		jQuery('.wp-smush-all, .wp-smush-scan').attr('disabled', 'disabled');
		$(".wp-smush-notice.wp-smush-remaining").hide();
		new _smush2.default($(this), true, 'nextgen');
	});
	...
	$('input#wp-smush-save-settings').on('click', function (e) {
		...
		if ('' != setting_type && 'network' == setting_type) {
			...
			if ((strip_exif === null || !strip_exif.checked) && (super_smush === null || !super_smush.checked) && (smush_original === null || !smush_original.checked) && (resize_images === null || !resize_images.checked) && (smush_pngjpg === null || !smush_pngjpg.checked) && (webp === null || !webp.checked)) {
			...
			_param = jQuery.param(_param) + '&' + jQuery('form#wp-smush-settings-form').serialize();

			// Send ajax, Update Settings, And Check For resmush.
			jQuery.post(ajaxurl, _param).done(function () {
				jQuery('form#wp-smush-settings-form').submit();
				return true;
			});
		}
	...
	$('body').on('click', '.wp-smush-skip-resmush', function (e) {
		...
		$.post(ajaxurl, params, function (res) {
			...
			if (res.success && 'undefined' !== typeof res.data.stats) {
				...
				// Smush notice.
				var remainingCountDiv = $('.bulk-smush-wrapper .wp-smush-remaining-count');
  ...
  */
	var scroll_and_enable_resize = function scroll_and_enable_resize() {
  ...
  */
	var scroll_and_enable_lossy = function scroll_and_enable_lossy() {
		// Enable super smush.
...
(function ($) {
    ...
    */
			$('button.wp-smush-all').on('click', function (e) {
				...
				var progress = $('.wp-smush-bulk-progress-bar-wrapper');
				/** @var {string} wp_smush_msgs.bulk_stop */
				progress.removeClass('wp-smush-exceed-limit').find('.sui-progress-close').attr('data-tooltip', wp_smush_msgs.bulk_stop);
				// Hide Resume button.
				...
				if ('undefined' === typeof wp_smushit_data || 0 === wp_smushit_data.unsmushed.length && 0 === wp_smushit_data.resmush.length) {
					return false;
				}
...
(function ($) {
    ...
    */
			$('#smush-enable-cdn').on('click', function () {
    ...
    */
			$('#smush-cancel-cdn').on('click', function (e) {
				e.preventDefault();
   ...
   * @param $enable
   */
		toggle_cdn: function toggle_cdn($enable) {
			var _this = this;
			...
			$.ajax({
				...
				if ('undefined' !== typeof resp.success && resp.success) {
					...
					_this.showNotice(resp.data.message);
				}
			}).error(function (resp) {
				var response = JSON.parse(resp.responseText);
				_this.showNotice(response.data.message);
			});
		},
   ...
   */
		showNotice: function showNotice(message) {
			...
			var notice = $('#wp-smush-ajax-notice');
...
var _jquery = __webpack_require__(/*! jquery.fancytree */ "./node_modules/jquery.fancytree/dist/modules/jquery.fancytree.js");

var _directoryScanner = __webpack_require__(/*! ../smush/directory-scanner */ "./_src/js/smush/directory-scanner.js");
...
(function ($) {
	...
	WP_Smush.directory = {
		...
		init: function init() {
			...
			var progress_dialog = $('#wp-smush-progress-dialog'),
			    totalSteps = 0,
			    currentScanStep = 0;
			...
			// Init image scanner.
			this.scanner = new _directoryScanner2.default(totalSteps, currentScanStep);
    ...
    */
			$('.wp-smush-select-dir').on('click', function (e) {
				...
				var selectedFolders = self.tree.getSelectedNodes(),
				...
				$.get(ajaxurl, param, function (response) {
					...
					self.scanner = new _directoryScanner2.default(response.data, 0);
    ...
    */
			progress_dialog.on('click', '.sui-icon-close, .sui-dialog-close, .wp-smush-cancel-dir', function (e) {
				...
				self.scanner.cancel().done(function () {
					return window.location.href = self.wp_smush_msgs.directory_url;
				});
			});
   ...
   * @returns {string}
   */
		getDirectoryList: function getDirectoryList() {
   ...
   * Init fileTree.
   */
		initFileTree: function initFileTree() {
			var self = this;

			var smushButton = $('button.wp-smush-select-dir');

			self.tree = (0, _jquery.createTree)('.wp-smush-list-dialog .content', {
				...
				lazyLoad: function lazyLoad(event, data) {
					return data.result = self.getDirectoryList(data.node.key);
				},
				loadChildren: function loadChildren(event, data) {
					return data.node.fixSelection3AfterClick();
				}, // Apply parent's state to new child nodes:
				select: function select() {
					return smushButton.attr('disabled', !+self.tree.getSelectedNodes().length);
				},
				init: function init() {
					return smushButton.attr('disabled', true);
				}
			});
		},
   ...
   */
		showSmushDialog: function showSmushDialog() {
   ...
   */
		showProgressDialog: function showProgressDialog(items) {
...
(function ($) {
   ...
   * @returns {*} Formatted Bytes
   */
		getSizeFromString: function getSizeFromString(formatted_size) {
   ...
   * @returns {*} Formatted Bytes
   */
		getFormatFromString: function getFormatFromString(formatted_size) {
   ...
   * @returns {number}
   */
		precise_round: function precise_round(num, decimals) {
			var sign = num >= 0 ? 1 : -1;
   ...
   * @returns {*}
   */
		geturlparam: function geturlparam(arg) {
...
var DirectoryScanner = function DirectoryScanner(totalSteps, currentStep) {
	...
	var obj = {
		scan: function scan() {
			...
			if (currentStep !== 0) {
				...
				$.post(ajaxurl, { action: 'directory_smush_start' }, function () {
		...
		limitReached: function limitReached() {
			var dialog = $('#wp-smush-progress-dialog');
...
var Smush = function () {
  ...
  */
	function Smush(button, bulk) {
		var type = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'media';

		_classCallCheck(this, Smush);
		...
		var progressBar = jQuery('.bulk-smush-wrapper .sui-progress-state-text');
		...
		var ids = 0 < wp_smushit_data.resmush.length && !this.skip_resmush ? wp_smushit_data.unsmushed.length > 0 ? wp_smushit_data.resmush.concat(wp_smushit_data.unsmushed) : wp_smushit_data.resmush : wp_smushit_data.unsmushed;
  ...
  */
   ...
   * Sync stats.
   */
		value: function sync_stats() {
			var message_holder = jQuery('div.wp-smush-bulk-progress-bar-wrapper div.wp-smush-count.tc');
			...
			jQuery.ajax({
				...
				return message_holder.html(progress_message);
			});
		}
	}, {
		key: 'bulk_done',
   ...
   */
		value: function bulk_done() {
			if (!this.is_bulk) return;

			// Enable the button.
			this.enable_button();

			var statusIcon = jQuery('.sui-summary-smush .smush-stats-icon');
			...
			if (0 === this.ids.length) {
				...
				var notice = jQuery('.bulk-smush-wrapper .wp-smush-resmush-notice');
   ...
   */
		value: function free_exceeded() {
			if (this.ids.length > 0) {
				var progress = jQuery('.wp-smush-bulk-progress-bar-wrapper');
				progress.addClass('wp-smush-exceed-limit').find('.sui-progress-block .sui-progress-close').addClass('sui-hidden').find('.sui-progress-block .wp-smush-all').removeClass('sui-hidden');
		...
		key: 'update_remaining_count',


		/**
   * Update remaining count.
   */
		value: function update_remaining_count() {
			if (this.is_bulk_resmush) {
				// Re-Smush notice.
				var resumeCountDiv = jQuery('.wp-smush-resmush-notice .wp-smush-remaining-count');
				if (resumeCountDiv.length && 'undefined' !== typeof this.ids) {
					resumeCountDiv.html(this.ids.length);
				}
			} else {
				// Smush notice.
				var wrapperCountDiv = jQuery('.bulk-smush-wrapper .wp-smush-remaining-count');
			...
			var sidenavCountDiv = jQuery('.smush-sidenav .wp-smush-remaining-count');
   ...
   * @param _res
   */
		value: function update_progress(_res) {
   ...
   * @private
   */
		value: function _update_progress(count, width) {
			...
			// Progress bar status.
			jQuery('.bulk-smush-wrapper .sui-progress-state-text').find('span:first-child').html(count).find('span:last-child').html(this.total);
		}
	}, {
		key: 'continue',
   ...
   */
		value: function _continue() {
			var continue_smush = this.button.attr('continue_smush');
   ...
   * @param {int} id
   */
		value: function increment_errors(id) {
			this.errors.push(id);
		}
	}, {
		key: 'increment_smushed',
   ...
   * @param {int} id
   */
		value: function increment_smushed(id) {
   ...
   */
		...
		value: function call_ajax() {
			...
			this.request = Smush.ajax(this.is_bulk_resmush, this.current_id, this.url, nonce_value).done(function (res) {
				...
				if ('undefined' === typeof res.success || 'undefined' !== typeof res.success && false === res.success && 'undefined' !== typeof res.data && 'limit_exceeded' !== res.data.error) {
					...
					var error_msg = Smush.prepare_error_row(res.data.error_message, res.data.file_name, res.data.thumbnail, self.current_id);
     ...
     */
				if ('undefined' !== typeof res.data && 'limit_exceeded' === res.data.error && !self.is_resolved()) {
					// Show error message.
					var bulk_error_message = jQuery('.wp-smush-bulk-progress-bar-wrapper');
					/** @var {string} res.data.error_message */
					bulk_error_message.find('.sui-notice-warning').html('<p>' + res.data.error_message + '</p>').show();
   ...
   */
		value: function run() {
			...
			if (this.is_bulk && this.ids.length > 0) this.call_ajax();

			if (!this.is_bulk) this.call_ajax();
		}
	}, {
		key: 'bind_deferred_events',


		/**
   * Show bulk Smush errors, and disable bulk Smush button on completion.
   */
		value: function bind_deferred_events() {
			var self = this;
			...
			this.deferred.done(function () {
				...
				if (self.errors.length) {
					...
					var msg = wp_smush_msgs.error_in_bulk.replace("{{errors}}", self.errors.length).replace("{{total}}", self.total).replace("{{smushed}}", self.smushed);

					jQuery('.wp-smush-all-done').addClass('sui-notice-warning').removeClass('sui-notice-success').find('p').html(msg);
				}
   ...
   */
		...
		key: 'smushAddParams',
		value: function smushAddParams(url, data) {
   ...
   */
		...
		value: function membership_validity(data) {
   ...
   */
		value: function ajax(is_bulk_resmush, id, send_url, nonce) {
   ...
   */
		value: function progress_bar(cur_ele, txt, state) {
   ...
   */
		value: function set_pro_savings() {
			// Default values.
   ...
   */
		value: function update_stats(scan_type) {
			...
			var formatted_size = WP_Smush.helpers.formatBytes(wp_smushit_data.savings_bytes, 1);
			var statsHuman = jQuery('.wp-smush-savings .wp-smush-stats-human');
			...
			var smushedCountDiv = jQuery('li.super-smush-attachments .smushed-count');
			...
			var smush_conversion_savings = jQuery('.smush-conversion-savings');
			...
			var smush_resize_savings = jQuery('.smush-resize-savings');
			...
			if ('undefined' !== typeof wp_smushit_data.pro_savings) {
				// Pro stats section.
				var smush_pro_savings = jQuery('.smush-avg-pro-savings');
   ...
   */
		...
		key: 'update_image_stats',
		value: function update_image_stats(new_size) {
			if (0 === new_size) {
				return;
			}

			var attachmentSize = jQuery('.attachment-info .file-size');
   ...
   * @param {string}  type
   */
		value: function update_localized_stats(image_stats, type) {
   ...
   * @returns {string}
   */
		value: function prepare_error_row(errorMsg, fileName, thumbnail, id) {
			var thumbDiv = 'undefined' === typeof thumbnail ? '<i class="sui-icon-photo-picture" aria-hidden="true"></i>' : thumbnail;
			var fileLink = 'undefined' === fileName || 'undefined' === typeof fileName ? 'undefined' : fileName;

			return '<div class="smush-bulk-error-row">' + '<div class="smush-bulk-image-data">' + thumbDiv + '<span class="smush-image-name">' + fileLink + '</span>' + '<span class="smush-image-error">' + errorMsg + '</span>' + '</div>' +
			/*
   ...
   * @param current_id
   */
		value: function update_smush_ids(current_id) {
			if ('undefined' !== typeof wp_smushit_data.unsmushed && wp_smushit_data.unsmushed.length > 0) {
