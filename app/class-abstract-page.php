...
abstract class Abstract_Page {
	...
	public function render() {
		...
		// Show Opt-in
		if ( ! get_site_option('submit_opt_in_modal') && ! WP_Smush::is_pro() ) {
			$this->view('opt-in', array(), 'modals');

		?>
		<script>
			window.addEventListener("load", function(){
				window.SUI.openModal( 'smush-opt-in-dialog', 'wpbody-content', undefined, false );
				});
		</script>
		<?php
		}
