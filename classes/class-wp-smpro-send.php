...
//			wp_die( $attachment_ID );
		...
		function process_meta_in_queue( $attachment_ID ) {
			...
			$this->queue_on_upload( $original_meta, $attachment_ID );
		 ...
		 */
		function send( $img_path = '', $img_url = '', $ID = 0, $size = '', $smush_meta ) {
...
//			error_log( json_encode( $data ) );
			...
			$smush_meta = $this->process_response( $data, $size, $smush_meta );
		 ...
		 * @return $metadata
		 */
		function process_response( $data, $size, $metadata ) {
			...
			if ( empty ( $metadata['smush_meta'] ) ){
				$metadata['smush_meta'] = array(
			...
			if ( ! empty( $file_id ) ) {
				...
				$metadata['smush_meta'][ $size ]['file_id']     = $file_id;
				$metadata['smush_meta'][ $size ]['status_code'] = $status_code;
				$metadata['smush_meta'][ $size ]['status_msg']  = $this->get_status_msg( $status_code, $request_err_code );
				$metadata['smush_meta'][ $size ]['token']       = $data->token;
				...
				//Return a error
				$metadata['smush_meta'][ $size ]['status_msg'] = "Unable to process the image, please try again later";
