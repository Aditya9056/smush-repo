...
if (!class_exists('WpSmPro')) {
        ...
        class WpSmPro {
                ...
                public function set_status($attachment_id, $status_type, $size, $state=0) {
                        error_log('got here: '.$status_type);
                 ...
                 */
                function update_stats($attachment_id){
                        $stats = array();
                        $statistics = array();
                        ...
                        $statistics['compressed_percent'] = ($statistics['compressed_bytes']/$statistics['before_smush'])*100;
				...
				$savings_str = number_format_i18n(
					( ( $statistics['compressed_bytes'] ) / 1024 ), 2 );
                        ...
                        if ( $statistics['compressed_bytes'] == 0 ) {
				$statistics['status_msg'] = __( 'Already Optimized', WP_SMPRO_DOMAIN );
			} else {
				$statistics['status_msg'] = sprintf( __( "Reduced by %01.1f%% (%s)", WP_SMPRO_DOMAIN ), $statistics['compressed_percent'], $savings_str . 'Kb' );
			} 
