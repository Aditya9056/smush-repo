        ...
        function custom_column($column_name, $id) {
            if ('smushit' == $column_name) {
                ...
                if (isset($data['smush_meta']) && !empty($data['smush_meta']['full'])) {
                    ...
                    printf("<br><a href=\"admin.php?action=wp_smpro_queue&amp;attachment_ID=%d\">%s</a>", $id, __('Re-smush', WP_SMUSHIT_PRO_DOMAIN));
                } else {
                    if (wp_attachment_is_image($id)) {
                        print __('Not processed', WP_SMUSHIT_PRO_DOMAIN);
                        printf("<br><a href=\"admin.php?action=wp_smpro_queue&amp;attachment_ID=%d\">%s</a>", $id, __('Smush.it now!', WP_SMUSHIT_PRO_DOMAIN));
                    }
                }
            }
        }
