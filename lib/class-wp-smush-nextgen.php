...
if( class_exists('WpSmushNextGen') ) {
	...
	//Extend Nextgen class to optimize dynamic thumbnails, generated by nextgen, check for auto smush
	if ( ! class_exists( 'WpSmushNextGenDynamicThumbs' ) && class_exists( 'Mixin' ) && $wpsmushnextgen->is_auto_smush_enabled() ) {
		...
		if( !$storage->get_wrapped_instance()->has_mixin('Mixin_NggLegacy_GalleryStorage_Driver')) {
			return;
		}
		class WpSmushNextGenDynamicThumbs extends Mixin_NggLegacy_GalleryStorage_Driver {
			...
			function generate_image_size( $image, $size, $params = null, $skip_defaults = false ) {
				...
				echo "</pre>";
				$success = Mixin_NggLegacy_GalleryStorage_Driver::generate_image_size( $image, $size, $params, $skip_defaults );
				if ( $success ) {
					echo "<pre>";
					print_r( $success );
					echo "</pre>";
