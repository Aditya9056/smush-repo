	...
	class WpSmushNextGenAdmin extends WpSmushNextGen {
		...
		function bulk_smush_content() {
			...
			if ( 0 >= $this->total_count ) : ?>
				...
				<p class="wp-smush-no-images-content tc"><?php printf( esc_html__( 'We havenвЂ™t found any images in your %1$sgallery%2$s yet, so thereвЂ™s no smushing to be done! Once you upload images, reload this page and start playing!', 'wp-smushit' ), '<a href="' . esc_url( admin_url( 'admin.php?page=ngg_addgallery' ) ) . '">', '</a>' ); ?></p>
