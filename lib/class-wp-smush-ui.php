	...
	class WpSmushBulkUi {
		...
		public function ui() {
			...
			if ( ! $is_network ) {
				...
				if ( ! $WpSmush->validate_install() ) {
					/**
					 * Allows to jook in additional containers after stats box for free version
		...
		public function container_header( $heading = '', $sub_heading = '' ) {
			...
			<div class="sui-box-header">
				<h3 class="sui-box-title"><?php echo $heading; ?></h3>
		...
		public function quick_setup() {
			...
			<div class="sui-dialog" id="smush-quick-setup-dialog">
				...
				<div class="sui-dialog-content sui-bounce-in" aria-labelledby="<?php esc_html_e( 'QUICK SETUP', 'wp-smushit' ); ?>" role="dialog">
		...
		public function bulk_smush_container() {
			...
			$smush_individual_msg = sprintf( esc_html__( 'Smush individual images via your %1$sMedia Library%2$s', 'wp-smushit' ), '<a href="' . esc_url( admin_url( 'upload.php' ) ) . '" title="' . esc_html__( 'Media Library', 'wp-smushit' ) . '">', '</a>' );
		...
		public function integrations_ui() {
			...
			// Container box class.
			$class = $WpSmush->validate_install() ? 'smush-integrations-wrapper wp-smush-pro' : 'smush-integrations-wrapper';
		...
		public function options_ui() {
			...
			<?php $class = $wpsmush_settings->settings['networkwide'] ? '' : ' hidden'; ?>
			...
			<form id="wp-smush-settings-form" method="post">
				...
				if ( ! is_multisite() || ( ! $wpsmush_settings->settings['networkwide'] && ! is_network_admin() ) || is_network_admin() ) {
					foreach( $this->intgration_group as $name ) {
						...
						// Show settings option.
						$this->settings_row( $setting_m_key, $label, $name, $setting_val, true );
					}
		 ...
		 * @return void
		 */
		public function settings_row( $setting_m_key, $label, $name, $setting_val, $skip_group = false ) {
		...
		public function bulk_smush_content() {
			...
			<?php else : ?>
				<div class="sui-notice sui-notice-success wp-smush-all-done<?php echo $all_done ? '' : ' hidden' ?>" tabindex="0">
				...
				if ( $wpsmushit_admin->remaining_count > 0 ) :
					...
					<div class="sui-notice sui-notice-warning<?php echo $class; ?>" tabindex="0">
						<p>
							...
							<?php if ( ! $WpSmush->validate_install() && $wpsmushit_admin->remaining_count > 50 ) : ?>
				...
				if ( ! $WpSmush->validate_install()  ) : ?>
					<p class="wp-smush-enable-lossy tc hidden"><?php esc_html_e( 'Tip: Enable Super-smush in the Settings area to get even more savings with almost no visible drop in quality.', 'wp-smushit' ); ?></p>
				<?php endif;
		...
		public function pro_features_container() {
			...
			global $WpSmush, $wpsmush_settings, $wpsmushit_admin;

			// Do not show if pro user.
			if ( $WpSmush->validate_install() || ( is_network_admin() && ! $wpsmush_settings->settings['networkwide'] ) ) {
				return;
			}
		...
		public function get_user_validation_message( $notice = true ) {
			...
			$content = sprintf( '<div id="wp-smush-invalid-member" data-message="%s" class="hidden' . $notice_class . '"><p>%s</p></div>', $attr_message, $message );
		...
		public function full_size_options( $name = '' ) {
			...
			if ( 'original' !== $name || ! $WpSmush->validate_install() ) {
				return;
			}
		...
		public function settings_desc( $setting_key = '' ) {
			...
			if ( empty( $setting_key ) || ! in_array( $setting_key, array( 'resize', 'original', 'keep_exif', 'png_to_jpg', 's3' ) ) ) {
				return;
			}
			?>
